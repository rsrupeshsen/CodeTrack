...
export function LoginPage() {
  ...
  const handleLogin = async (e: FormEvent) => {
    ...
    try {
      ...
      // ✅ Reload profile from Appwrite so dashboard shows data immediately
      await reloadUser();
      navigate("/dashboard");
    } catch (err: any) {
    ...
    <div className="min-h-screen flex pt-16">
      <div className="hidden lg:flex flex-1 bg-gradient-to-br from-primary/20 to-teal-600/10 items-center justify-center p-12 relative overflow-hidden">
        ...
        <div className="relative z-10 max-w-md text-center">
          ...
          <h2 className="text-3xl text-foreground mb-4" style={{ fontWeight: 700 }}>Welcome back to CodeFolio</h2>
          <p className="text-muted-foreground text-lg mb-8">Track your coding progress across platforms and showcase your developer journey.</p>
          <div className="grid grid-cols-3 gap-4">
            {[{ val: "1,247", label: "Problems" }, { val: "1,856", label: "Rating" }, { val: "42", label: "Repos" }].map((s) => (
      ...
      <div className="flex-1 flex items-center justify-center p-8 bg-background">
        <div className="w-full max-w-md">
          ...
          <h1 className="text-2xl text-foreground mb-2" style={{ fontWeight: 700 }}>Sign in to your account</h1>
          ...
          <form onSubmit={handleLogin} className="space-y-5">
            ...
            {/* ✅ Wired to real ForgotPasswordPage */}
            <div className="flex justify-end">
              <Link to="/forgot-password" className="text-primary text-sm hover:underline" style={{ fontWeight: 500 }}>
                Forgot password?
              </Link>
            </div>
            ...
            <button type="submit" disabled={isLoading}
              className="w-full bg-primary text-primary-foreground py-3 rounded-xl hover:bg-primary/90 transition-all cursor-pointer disabled:opacity-70 flex items-center justify-center gap-2"
          ...
          <p className="text-center text-muted-foreground text-sm mt-6">
            ...
            <Link to="/signup" className="text-primary hover:underline" style={{ fontWeight: 500 }}>Sign up</Link>
          </p>
        </div>
      </div>
    </div>
