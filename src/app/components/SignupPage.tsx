...
import { Code2, Eye, EyeOff, Loader2, Mail, CheckCircle2 } from "lucide-react";
...
export function SignupPage() {
  ...
  // ✅ After signup, show "check your email" instead of immediately navigating
  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Show the "check your email" screen instead of going straight to onboarding
      setSignedUp(true);
    } catch (err: any) {
  ...
  const handleGoogleSignup = async () => {
    ...
    catch (err: any) { setError(err?.message || "Google sign up failed. Please try again."); }
  };

  // ── Post-signup: prompt user to verify email ──────────────────────────────
  if (signedUp) {
    return (
        ...
        <div className="w-full max-w-md text-center">
          ...
          <div className="bg-card border border-border rounded-2xl p-8">
            ...
            <p className="text-muted-foreground text-sm leading-relaxed mb-6">
              Click the link in that email to verify your account, then you can start setting up your profile.
            </p>
    ...
    <div className="min-h-screen flex pt-16">
      <div className="hidden lg:flex flex-1 bg-gradient-to-br from-primary/20 to-teal-600/10 items-center justify-center p-12 relative overflow-hidden">
        ...
        <div className="relative z-10 max-w-md text-center">
          ...
          <h2 className="text-3xl text-foreground mb-4" style={{ fontWeight: 700 }}>Join CodeFolio</h2>
          <p className="text-muted-foreground text-lg mb-8">
            Create your developer portfolio in minutes. Connect your coding profiles and start tracking your progress.
          </p>
          <div className="flex flex-wrap justify-center gap-2">
            {["LeetCode", "GeeksForGeeks", "GitHub", "Analytics", "AI Assistant"].map((tag) => (
              <span key={tag} className="bg-primary/10 border border-primary/20 text-primary px-3 py-1 rounded-full text-sm">{tag}</span>
            ))}
          </div>
        </div>
      </div>
      ...
      <div className="flex-1 flex items-center justify-center p-8 bg-background">
        <div className="w-full max-w-md">
          <div className="lg:hidden flex items-center gap-2 mb-8 justify-center">
            <Code2 className="w-7 h-7 text-primary" />
            <span className="text-xl text-foreground" style={{ fontWeight: 700 }}>Code<span className="text-primary">Folio</span></span>
          </div>

          <h1 className="text-2xl text-foreground mb-2" style={{ fontWeight: 700 }}>Create your account</h1>
          <p className="text-muted-foreground mb-8">Get started with your free CodeFolio account</p>
          ...
          <form onSubmit={handleSignup} className="space-y-4">
            ...
            <div>
              ...
              {form.password.length > 0 && (
                <div className="mt-2">
                  ...
                  <p className="text-xs mt-1" style={{ color: strength.color, fontWeight: 500 }}>{strength.label}</p>
                </div>
              )}
            </div>
            <div>
            ...
            {error && (
              <div className="bg-destructive/10 border border-destructive/30 text-destructive rounded-xl px-4 py-3 text-sm">{error}</div>
            )}
          ...
          <p className="text-center text-muted-foreground text-sm mt-6">
            ...
            <Link to="/login" className="text-primary hover:underline" style={{ fontWeight: 500 }}>Sign in</Link>
          </p>
        </div>
      </div>
    </div>
